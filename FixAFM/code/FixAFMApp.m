classdef FixAFMApp < handle
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            FixAFMApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'StepReduce_V4')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong StepReduce_V4
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'StepReduce_V4').getString, ...
            ...
            if nargout(@StepReduce_V4) == 0
                eval('StepReduce_V4');
            else
                obj.AppHandle = eval('StepReduce_V4');
            end
