classdef RotateAndCropApp < handle
    %
    % Usage:
    %     app.RotateAndCropApp
    ...
    properties (Constant)
        AppPathBase = {'/RotateAndCrop/code'};
        AppClass = 'RotateAndCropApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            RotateAndCropApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'RotateCrop')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong RotateCrop
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'RotateCrop').getString, ...
            ...
            if nargout(@RotateCrop) == 0
                eval('RotateCrop');
            else
                obj.AppHandle = eval('RotateCrop');
            end
